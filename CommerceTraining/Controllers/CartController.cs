    ...
    public class CartController : OrderControllerBase<CartPage>
    {
        ...
        public ActionResult Index(CartPage currentPage, string routedData)
        {
            ...
            else
            {
                ...
                // added for Shipping test ... can use this
                //Guid g = cart.GetFirstShipment().ShippingMethodId;
                //if (g == new Guid())
                //{
                //    var s = "still the same";
                //}

                // may need/want this... or not... playing around some
                ...
                // New Tax
                //var t = _orderGroupFactory.CreateTaxValue(cart);
                
                //_taxCalc.Service.GetTaxTotal(cart.GetFirstForm()
                //    , _currentMarket.GetCurrentMarket(), cart.Currency);
                ...
                var disc = _orderGroupCalculator.GetOrderDiscountTotal(cart, cart.Currency);

                var model = new CartViewModel
                {
                    LineItems = cart.GetAllLineItems(), // Extension method
                    CartTotal = _orderGroupCalculator.GetSubTotal(cart) - _orderGroupCalculator.GetOrderDiscountTotal(cart, cart.Currency),
        ...
        private string CheckApplicableShippings(string code)
        {
            ...
            // would like to index the "right method" guid directly on the Entry... perhaps
        ...
        public void DeadEnd(string id)
        {
            try
            {
                id += "...it works with CMS-refs, not for Commerce";
            }
